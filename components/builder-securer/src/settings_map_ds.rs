...
use super::SettingsMapOutput;

pub struct SettingsMapDS;

impl SettingsMapDS {
    pub fn create(datastore: &DataStoreConn, settings_map_create: &settings_map::SettingsMap) -> SettingsMapOutput {
    ...
    pub fn show(datastore: &DataStoreConn, get_settingsmap: &base::IdGet) -> SettingsMapOutput {
        let conn = datastore.pool.get_shard(0)?;
        let rows = &conn.query(
    ...
    pub fn show_by_id(datastore: &DataStoreConn, get_settingsmap: &IdGet) -> SettingsMapOutput {
        let conn = datastore.pool.get_shard(0)?;
        let rows = &conn.query(
