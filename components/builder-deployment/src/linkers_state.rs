...
impl ServiceState {
    ...
    pub fn new(state_ids: Vec<af_ids>, conn: &DataStoreConn) -> ServiceState {
        ...
        let linkers = state_ids.map(|id| {
            let assemblys = AssemblyFactory::describe(conn, &IdGet { id })?;
...
fn main() {
    ...
    let dns = Service::new(); //create a "From" to Service
    let l = LinkersState::new(ids, conn);
    l.add_dns_connection(dns);

  
    //// Usecase 2: Remove
    let lb = Service::new(); //create a "From" to Service
