...
// use std::str::FromStr;
// use protocol::{jobsrv, servicesrv};
...
//
//
//     pub fn extend(&mut self, service: &servicesrv::Service) -> (usize, usize) {
//         let name = format!("{}", service.get_id());
//         let (srv_id, srv_node) = self.generate_id(&name);
//
//         assert_eq!(srv_id, srv_node.index());
//
