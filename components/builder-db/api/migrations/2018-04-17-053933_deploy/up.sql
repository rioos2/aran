...
CREATE TABLE IF NOT EXISTS blockchain_factory (id bigint PRIMARY KEY DEFAULT next_id_v1('blc_fact_id_seq'),
                                                                           object_meta JSONB,
                                                                                       type_meta JSONB,
                                                                                                 replicas smallint, resources JSONB,
                                                                                                                              metadata JSONB,
...
CREATE OR REPLACE FUNCTION insert_blockchain_factory_v1 (object_meta JSONB, type_meta JSONB, replicas smallint, resources JSONB, metadata JSONB, status JSONB, secret JSONB, PLAN bigint, spec JSONB) RETURNS
SETOF blockchain_factory AS $$
BEGIN
RETURN QUERY INSERT INTO blockchain_factory(object_meta,type_meta,replicas,resources,metadata,status,secret,plan,spec)
VALUES (object_meta,type_meta,replicas,resources,metadata,status,secret,plan,spec)
RETURNING *;
RETURN;
END
...
CREATE OR REPLACE FUNCTION get_blockchain_factory_v1 (aid bigint) RETURNS
SETOF blockchain_factory AS $$
BEGIN
RETURN QUERY SELECT * FROM blockchain_factory WHERE id = aid;
RETURN;
END
$$ LANGUAGE PLPGSQL STABLE;


CREATE OR REPLACE FUNCTION get_blockchains_factory_v1() RETURNS
SETOF blockchain_factory AS $$
BEGIN
RETURN QUERY SELECT * FROM blockchain_factory;
RETURN;
END
$$ LANGUAGE PLPGSQL STABLE;


CREATE OR REPLACE FUNCTION set_blockchain_factorys_status_v1 (aid bigint, asm_fac_status JSONB) RETURNS
SETOF blockchain_factory AS $$
BEGIN
RETURN QUERY UPDATE blockchain_factory SET status=asm_fac_status, updated_at=now() WHERE id=aid
RETURNING *;
RETURN;
END
$$ LANGUAGE PLPGSQL VOLATILE;
...
CREATE OR REPLACE FUNCTION get_blockchain_factory_by_account_v1 (account_id text) RETURNS
SETOF blockchain_factory AS $$
BEGIN
RETURN QUERY SELECT * FROM blockchain_factory WHERE object_meta ->> 'account' = account_id;
RETURN;
END
$$ LANGUAGE PLPGSQL STABLE;
