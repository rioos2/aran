...
VALUES (
          (SELECT id
           FROM first_insert), 'rioos.assembly.get',
                               'Read only access to all the users  VMs, Containers'),(
                                                                                        (SELECT id
                                                                                         FROM first_insert), 'rioos.assembly.list',
                                                                                                             'Read only access to all the users  VMs, Containers');

WITH first_insert AS
  (INSERT INTO roles(name, description)
   VALUES('rioos:universalsoldier',
          'universal soldier of RIO/OS. God given powers.  instance') ON CONFLICT (name) DO NOTHING RETURNING id)
INSERT INTO permissions (role_id, name, description)
VALUES (
          (SELECT id
           FROM first_insert), 'rioos.assembly.get',
...
WITH second_insert AS
  (INSERT INTO roles(name, description)
   VALUES('rioos:loneranger',
          'TeamOwner of RIO/OS team') ON CONFLICT (name) DO NOTHING RETURNING id)
INSERT INTO permissions (role_id, name, description)
VALUES (
          (SELECT id
           FROM second_insert), 'rioos.assembly.get',
                                'Read only access to all the users  VMs, Containers'),(
                                                                                         (SELECT id
                                                                                          FROM second_insert), 'rioos.assembly.get',
                                                                                                               'Read only access to all the users  VMs, Containers');
