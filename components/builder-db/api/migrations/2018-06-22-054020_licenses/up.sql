...
CREATE TABLE IF NOT EXISTS LICENSES (id bigint PRIMARY KEY DEFAULT next_id_v1('license_id_seq'),
                                                                   object_meta JSONB,
                                                                               type_meta JSONB,
                                                                                         status text, license_id text,password text,expired text, activation JSONB,user_activation bool, provider text, error text,product text,
...
CREATE FUNCTION insert_or_update_license_v1(lobject_meta JSONB, ltype_meta JSONB, lstatus text, lexpired text, lactivation JSONB,luser_activation bool,lprovider text,lerror text,lproduct text)RETURNS
SETOF LICENSES AS $$
...
BEGIN
   ...
   WHERE
      ...
      WHERE
         ...
         ELSE
         RETURN QUERY
         INSERT INTO LICENSES(object_meta,type_meta, status,expired, activation, user_activation,provider,error,product)
           VALUES(lobject_meta, ltype_meta, lstatus,lexpired ,lactivation, luser_activation,lprovider,lerror,lproduct)ON CONFLICT DO NOTHING RETURNING *;
      RETURN;
      END
      IF;
      RETURN;
