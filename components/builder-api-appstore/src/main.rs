...
//! ~~~~ This where everything starts: main starting point of the Rio.AppStore server.
//!
#[macro_use]
extern crate clap;
extern crate env_logger;
extern crate rio_appstore_api as api;
...
fn app<'a, 'b>() -> clap::App<'a, 'b> {
    clap_app!(RIOOSAran =>
        (version: api::VERSION)
        (about: "Rio.AppStore api-server")
        (@setting VersionlessSubcommands)
        (@setting SubcommandRequiredElseHelp)
        (@subcommand start =>
            (about: "Run the rio.appstore server")
            (@arg config: -c --config +takes_value
                "Filepath to configuration file. [default: /var/lib/rioos/config/appstore.toml]")
