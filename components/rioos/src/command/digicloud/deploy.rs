...
pub fn start(ui: &mut UI, rio_client: Client, cache_path: &str, token: &str, email: &str) -> Result<()> {
    ...
    if let Some(i) = content.horizontal_scaling {
        ...
        let ref mut object_data = hscale.mut_meta(hscale.object_meta(), hscale.object_meta().name, hscale.object_meta().account);

        hscale.set_owner_reference(object_data, assembly_fac.type_meta().kind, assembly_fac.type_meta().api_version, hscale.object_meta().name, assembly_fac.get_id());
        hscale.set_meta(type_meta_url("".to_string()), object_data.clone());

        rio_client.create_horizontal_scaling(hscale.clone(), token, email)?;
    }
