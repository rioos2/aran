...
impl<C: TlsConnector> ClientBuilder<C> {
    ...
    pub fn build(self) -> Result<Client> {
        ...
        let join = if let Some(remote) = self.event_loop {
            ...
            let tls = self.tls;
            let conf = self.conf;
            let thread_name = conf.thread_name
                .clone()
...
impl Client {
    ...
    pub fn start_request_simple(&mut self, headers: Headers, body: Bytes) -> Response {
        self.start_request(headers, HttpPartStream::once_bytes(body))
    }

    pub fn start_get(&mut self, path: &str, authority: &str) -> Response {
    ...
    pub fn start_post(&mut self, path: &str, authority: &str, body: Bytes) -> Response {
        let headers = Headers(vec![
...
impl Service for Client {
    ...
    fn start_request(&self, headers: Headers, body: HttpPartStream) -> Response {
        ...
        if let Err(_) = self.controller_tx
...
fn spawn_client_event_loop<T: ToClientStream + Send + Clone + 'static, C: TlsConnector>(handle: reactor::Handle, shutdown_future: ShutdownFuture, socket_addr: T, tls: ClientTlsOption<C>, conf: ClientConf, done_tx: oneshot::Sender<()>, controller_tx: UnboundedSender<ControllerCommand>, controller_rx: UnboundedReceiver<ControllerCommand>) {
    ...
    handle.spawn(conn_future.map_err(|e| {
        warn!("client error: {:?}", e);
        ()
    }));
