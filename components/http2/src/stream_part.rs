...
pub struct HttpPartStream(pub HttpFutureStreamSend<HttpStreamPart>);
...
impl HttpPartStream {
    ...
    where
        S: Stream<Item = HttpStreamPart, Error = error::Error> + Send + 'static,
    {
    ...
    pub fn once(part: HttpStreamPartContent) -> HttpPartStream {
        HttpPartStream::new(stream::once(Ok(HttpStreamPart {
            content: part,
            last: true,
        })))
    }
    ...
    pub fn filter_data(self) -> HttpFutureStreamSend<Bytes> {
        Box::new(
            self.filter_map(|HttpStreamPart { content, .. }| match content {
    ...
    pub fn check_only_data(self) -> HttpFutureStreamSend<Bytes> {
        Box::new(
            self.and_then(|HttpStreamPart { content, .. }| match content {
                HttpStreamPartContent::Data(data) => Ok(data),
                HttpStreamPartContent::Headers(..) => Err(error::Error::from(io::Error::new(
                    io::ErrorKind::Other,
                    "expecting only DATA frames",
                ))),
            }),
        )
    }
