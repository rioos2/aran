        ...
        let stream_id = self.inner.with(move |inner: &mut ClientInner| {
            ...
            let out_window = {
                ...
                http_stream
                    .stream()
                    .outgoing
                    .push_back(HttpStreamPartContent::Headers(headers));
...
impl ClientConnection {
        ...
        C: ClientConnectionCallbacks,
    {
        ...
        let future = handshake.and_then(move |conn| {
            ...
            let run_write = ClientWriteLoop {
                write: write,
                inner: inner.clone(),
            }.run(to_write_rx);
            let run_read = ClientReadLoop {
                read: read,
                inner: inner.clone(),
            }.run();
            let run_command = ClientCommandLoop {
                inner: inner.clone(),
            }.run(command_rx);
    ...
    pub fn dump_state_with_resp_sender(&self, tx: oneshot::Sender<ConnectionStateSnapshot>) {
        ...
        drop(
            self.command_tx
                .unbounded_send(ClientCommandMessage::DumpState(tx)),
        );
    }
