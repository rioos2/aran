            ...
            match state {
                ...
                DecodedRef::U2(OPEN) => {
                    if let Ok(_) = self.shared
                        .state
                        .compare_int_exchange(OPEN, DecodedBox::U2(CLOSED_NO_WAIT))
                    {
                        return;
                    }
                }
...
impl Latch {
    pub fn poll_ready(&self) -> Poll<(), ControllerDead> {
        loop {
            ...
            match self.shared
                .state
                .compare_exchange(s, DecodedBox::Box(Box::new(task::current())))
            {
...
mod test {
    ...
    fn assert_notify_stream<S>(expected: Poll<Option<S::Item>, S::Error>, stream: &mut Spawn<S>)
    where
        S: Stream,
        S::Item: Debug,
        S::Item: PartialEq,
        ...
        S::Error: PartialEq,
    {
        assert_eq!(expected, stream.poll_stream_notify(&notify_noop(), 1));
    }
