...
pub type HttpFutureSend<T> = ::std::boxed::Box<Future<Item = T, Error = Error> + Send>;
pub type HttpFutureStreamSend<T> = ::std::boxed::Box<Stream<Item = T, Error = Error> + Send>;
...
pub fn recv_raw_frame<'r, R: AsyncRead + 'r>(
    ...
    let frame_buf = header.and_then(
        move |(read, raw_header)| -> Box<Future<Item = _, Error = _> + 'r> {
            ...
            if header.length > max_frame_size {
                ...
                return Box::new(future::err(error::Error::CodeError(
    ...
    let frame = frame_buf.map(|(read, frame_buf)| (read, RawFrame::from(frame_buf.vec)));
    Box::new(frame)
}
...
pub fn recv_http_frame_join_cont<'r, R: AsyncRead + 'r>(
    ...
    Box::new(loop_fn::<(R, Option<ContinuableFrame>), _, _, _>(
        (read, None),
        move |(read, header_opt)| {
            recv_http_frame(read, max_frame_size).and_then(move |(read, frame)| match frame {
                HttpFrame::Headers(h) => {
                ...
                HttpFrame::PushPromise(p) => {
                    if let Some(_) = header_opt {
                        ...
                        if p.flags.is_set(PushPromiseFlag::EndHeaders) {
                            ...
                            Ok(Loop::Continue((
                                read,
                                Some(ContinuableFrame::PushPromise(p)),
                            )))
                        }
                    }
                }
        ...
        recv_settings_frame(read, max_frame_size).and_then(|(read, frame)| {
            if frame.is_ack() {
                ...
                Err(Error::InvalidFrame(
                    "expecting SETTINGS with ack, got without ack".to_owned(),
                ))
            }
        }),
    )
    ...
    Box::new(
        recv_settings_frame(read, max_frame_size).and_then(|(read, frame)| {
            if !frame.is_ack() {
                ...
                Err(Error::InvalidFrame(
                    "expecting SETTINGS without ack, got with ack".to_owned(),
                ))
            }
        }),
    )
...
pub fn send_frame<W: AsyncWrite + Send + 'static, F: FrameIR>(write: W, frame: F) -> HttpFuture<W> {
    ...
    Box::new(write_all(write, buf).map(|(w, _)| w).map_err(|e| e.into()))
}
