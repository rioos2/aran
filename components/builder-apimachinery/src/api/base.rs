...
pub struct InitializerStatus {
    ...
    pub status: String, // Status of the operation.// One of: "Success" or "Failure"
    pub message: String, // A human-readable description of the status of this operation.
    pub reason: String, // A machine-readable description of why this operation is in the "Failure" status. If this value is empty there is no      information available. A Reason clarifies an HTTP status  code but does not override it.
    //StatusDetails is a set of additional properties that MAY be set by the
    // server to provide additional information about a response.
    pub details: StatusDetails,
    pub code: u32, // Suggested HTTP return code for this status, 0 if not set.
}

#[derive(Debug, PartialEq, Clone, Default, Serialize, Deserialize)]
pub struct StatusDetails {
    pub name: String, //The name attribute of the resource associated with the status StatusReason
    pub group: String, //The group attribute of the resource associated with the status StatusReason.
    pub kind: String, //The kind attribute of the resource associated with the status StatusReason.
    pub causes: Vec<StatusCause>, // The Causes array includes more details associated with the StatusReason failure. Not all StatusReasons may provide detailed causes.
    pub uid: String, //UID of the resource.
    pub retry_after_seconds: u32, //If specified, the time in seconds before the operation should be retried.
}

#[derive(Debug, PartialEq, Clone, Default, Serialize, Deserialize)]
pub struct StatusCause {
    pub cause_type: String, // A machine-readable description of the cause of the error. If this value is empty there is no information available.
    pub message: String, //A human-readable description of the cause of the error.  This field may be presented as-is to a reader.
    pub field: String, // The field of the resource that has caused this error, as named by its JSON serialization. May include dot and postfix notation for nested attributes."name" - the field "name" on the current resource  "items[0].name" - the field "name" on the first array entry in "items".
}
