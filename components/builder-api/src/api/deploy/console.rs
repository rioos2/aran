        ...
        match assembly::DataStore::new(&self.conn).show(&id_get) {
            ...
            Ok(Some(assembly)) => {
                if !assembly.get_metadata().contains_key("rioos_sh_vnc_host") || !assembly.get_metadata().contains_key("rioos_sh_vnc_port") {
                        ...
                        let vnc = &"".to_string();
                    let host = assembly.get_metadata().get("rioos_sh_vnc_host").unwrap_or(vnc);
                    let port = assembly.get_metadata().get("rioos_sh_vnc_port").unwrap_or(vnc);
                    ...
                    match res {
                        Ok(mut data) => {
                            ...
                            Ok(render_json(status::Ok, &ExecURL {
